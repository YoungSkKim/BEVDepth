...
from datasets.nusc_det_dataset_radar import NuscDetDatasetRadar, collate_fn
...
backbone_pts_conf = {
    ...
    'pts_middle_encoder':
    dict(
        type='PointPillarsScatter', in_channels=64, output_shape=(256, 256)  # TODO: shape to variable/
    ),
    'pts_backbone':
    dict(
        type='SECOND',
...
class BEVDepthLightningModel(LightningModule):
    ...
    def __init__(self,
        ...
        # self.basic_lr_per_img = 2e-4 / 24  # lr for batch 6, GPU 4
    ...
    def train_dataloader(self):
        train_dataset = NuscDetDatasetRadar(
            ...
            return_radar_fv=True,
            return_radar_bev=True,
            radar_path='radar_filter')
        ...
        train_loader = torch.utils.data.DataLoader(
            ...
            collate_fn=partial(collate_fn,
                               ...
                               is_return_radar_fv=True,
                               is_return_radar_bev=True),
            sampler=None,
        )
        return train_loader

    def val_dataloader(self):
        val_dataset = NuscDetDatasetRadar(
            ida_aug_conf=self.ida_aug_conf,
            bda_aug_conf=self.bda_aug_conf,
            classes=self.class_names,
            data_root=self.data_root,
            ...
            sweep_idxes=self.sweep_idxes,
            key_idxes=self.key_idxes,
            return_depth=self.data_return_depth,  # TODO
            return_radar_fv=True,
            return_radar_bev=True,
            radar_path='radar_filter')
    ...
    def predict_dataloader(self):
        predict_dataset = NuscDetDatasetRadar(
            ida_aug_conf=self.ida_aug_conf,
            bda_aug_conf=self.bda_aug_conf,
            classes=self.class_names,
            data_root=self.data_root,
